...
public class WalkToTargetTask extends Task<MobEntity> {
   ...
   protected void resetTask(ServerWorld worldIn, MobEntity entityIn, long gameTimeIn) {
      if (entityIn.getBrain().hasMemory(MemoryModuleType.WALK_TARGET) && !this.hasReachedTarget(entityIn, entityIn.getBrain().getMemory(MemoryModuleType.WALK_TARGET).get())) {
